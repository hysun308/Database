...
public class LockManager {
  ...
  private void handlePotentialDeadlock(Lock lock, long transNum, LockType lockType) {
    ...
    for(long to : lock.getOwners()){
      if(this.waitsForGraph.edgeCausesCycle(transNum,to)&&(!lock.checkCompatible(transNum,lockType))){
        throw new DeadlockException("There is a DeadLock!");
      }
    }

    this.waitsForGraph.addNode(transNum);
    for(long node : lock.getOwners()) {
      if (node != transNum&&(!lock.checkCompatible(transNum,lockType))) {
        this.waitsForGraph.addEdge(transNum, node);
      }
    }
    return;
  }
