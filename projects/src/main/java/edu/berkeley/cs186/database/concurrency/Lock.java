...
public class Lock {
  ...
  public boolean checkCompatible (long transNum,LockManager.LockType lockType){

      if(this.containsTransaction(transNum)&&(this.type.equals(LockManager.LockType.EXCLUSIVE)||lockType.equals(LockManager.LockType.SHARED))){
          return true;
      }
      if(this.type.equals(LockManager.LockType.SHARED)&&lockType.equals(LockManager.LockType.SHARED)&&(!this.getOwners().isEmpty())){
          return true;
      }
      LockRequest lockRequest = new LockRequest(transNum,lockType);
      ...
      if((!this.transactionQueue.isEmpty())&&this.transactionQueue.peek().equals(lockRequest)&&this.getOwners().isEmpty()){
          return true;
      }
      if(lockType.equals(LockManager.LockType.SHARED)&&this.transactionQueue.isEmpty()){
