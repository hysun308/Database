...
public class TestDeadlockPrevention {
  ...
  public void testTwoTransactionNoDeadlock() throws InterruptedException {
  ...
  @Test
  @Category(StudentTestP3.class)
  public void testSimpleThreeTransactionDeadLock () throws InterruptedException{
      final LockManager lockMan = new LockManager();
      AsyncDeadlockTesterThread thread1 = new AsyncDeadlockTesterThread(new Runnable() {
          public void run() {
              lockMan.acquireLock("A", 1, LockManager.LockType.SHARED);
      ...
      try {
          ...
          thread4.join(100); //waits for thread to finish (timeout of .1 sec)
          thread4.test();
          assertTrue("Transaction 3 Thread should not have finished", thread4.isAlive()); //T3 should be waiting on T2 for B
          ...
          thread6.join(100); //waits for thread to finish (timeout of .1 sec)
          thread6.test();
          assertFalse("Transaction 1 Second Thread should not have finished", thread6.isAlive()); //T1 should be waiting on T3 for C
  ...
  @Test
  @Category(StudentTestP3.class)
  public void testThreeTransactionNoDeadLock() throws InterruptedException{
      final LockManager lockMan = new LockManager();
      AsyncDeadlockTesterThread thread1 = new AsyncDeadlockTesterThread(new Runnable() {
          public void run() {
              lockMan.acquireLock("A", 1, LockManager.LockType.EXCLUSIVE);
      ...
      try {
          ...
          thread2.join(100); //waits for thread to finish (timeout of .1 sec)
          thread2.test();
          assertTrue("Transaction 2 Thread should have not have finished", thread2.isAlive()); //T2 should be blocked

          thread3.start();
          thread3.join(100); //waits for thread to finish (timeout of .1 sec)
          thread3.test();
          assertTrue("Transaction 3 Thread should not have finished", thread3.isAlive()); //T3 should be blocked
      } catch (DeadlockException d) {
          fail("No deadlock exists but Deadlock Exception was thrown.");
      }
  }
